...
The two boards ship as **two separate installable PWAs** off the one build: the
Maths board at the deployment root, the Language board under `/language/`. Each
...
the same build works at the site root (the VPS) and under the repo subpath
(GitHub Pages). A single service worker (`public/sw.js`) is registered from
`src/pwa.ts` at the deployment root — one worker, root scope, covering both
apps — giving offline load (network-first for pages, stale-while-revalidate for
assets; `/api/` and `/ys/` are never intercepted). It registers in production
builds only.
