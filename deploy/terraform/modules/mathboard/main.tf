# mathboard stack module
# -----------------------
# One small OpenStack instance running the whole compose stack (Caddy + token
# API + Y-Sweet, optionally self-hosted Umami analytics) plus an S3-compatible
# bucket. Instantiated once per environment: the production root
# (deploy/terraform) and the dev/staging root (deploy/terraform/dev) both call
# this module with a different `name`, so their resources never collide.
#
# The provider itself is configured by the ROOT module and inherited here.
terraform {
...
# Each environment gets its OWN bucket (var.s3_bucket differs), so dev traffic
# never touches production documents or uploads.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Analytics secrets
# Generated so an analytics-enabled apply comes up ready (self-hosted Umami +
...
# They are always generated (cheap, no external calls) but only written into the
# instance .env when var.enable_analytics is true - dev leaves analytics off and
